...
export const publications = [
  {
    ...
    "sections": [
      {
        ...
        "body": "A final answer records that a solution worked. It does not show how the solver tracked the state, rejected the alternatives that looked promising, or anticipated a consequence several moves away. Human process supervision can expose those operations, but it is [c:red]expensive, hard to verify, and capped by the expertise of the annotators[/c]. Letting a model generate its own supervision scales better, yet ties the quality of the data to the errors the learner already makes.\n\n:::key Self-Play Search Distillation\n**SPSD** draws its supervision from a [c:green]frozen search expert[/c] that already plays above human level.\n\nIn choosing a move, such an expert compares legal alternatives, scores them, and explores their consequences. We retain those comparisons and convert them into reasoning supervision for a language model.\n:::\n\nBoard games make this practical. The rules define [c:gold]exact transitions and legal actions[/c], terminal states expose delayed consequences, and **every claim in a training example can be checked by replaying it in the simulator**. The expert supplies the decisions; replay supplies the facts used to explain them.\n\n{{visual:Figure 1}}\n\nGame competence tracks broader reasoning performance as models grow, and even frontier systems are far from saturating the board-game benchmark. These environments demand exactly what a scale curve cannot supply on its own: [c:blue]maintaining state[/c] and [c:blue]anticipating consequences[/c] across a sequence of decisions.\n\n:::claim Evaluation targets\nWe train on game-derived supervision and measure what transfers to **unseen games and mathematics**, neither of which the corpus contains.\n:::\n\nThe panels above use GPQA-Diamond and Humanity's Last Exam for cross-model context; the post-training experiments use a separate suite of six mathematics benchmarks."
      },
      {
        "id": "spsd-search-to-supervision",
        "title": "Replay turns search records into training data",
        "body": "SPSD runs in **three stages**: a frozen search expert plays itself and exports one record per decision state, the simulator [c:green]replays every record and discards whatever it cannot reproduce[/c], and a renderer converts the survivors into prompts and reference responses.\n\n:::note Freezing the expert\nFreezing the expert before generation keeps the corpus **independent of the student trained from it**. There is no feedback loop, and no drift toward the learner's own errors.\n:::\n\nEach game gets an EfficientZero expert trained by self-play through LightZero. At every retained state it runs [c:pink]50 Monte Carlo tree search simulations[/c] and exports the selected action, the visit-derived ranking over legal alternatives, [c:cyan]value estimates[/c], and replayable branch evidence. Trajectories start from a uniformly random legal prefix, so retained decisions spread across the reachable state space instead of clustering on one deterministic line of play.\n\n{{visual:Figure 2}}\n\nThe renderer restores the position and replays the expert's move, up to one visit-ranked alternative, and where available one opponent reply. **Only board effects recovered from those transitions reach the trace.**\n\n:::warn Replay constraint\nA win, loss, or draw is stated [c:red]only when replay reaches that terminal state[/c]. Invalid optional branches are dropped, and a record whose selected action fails to replay is **rejected**. Every chain ends on the expert's chosen legal handle.\n:::\n\nChoosing a strong move is not the same as reading the board that justifies it, so SPSD adds six **state questions**: occupancy, legality, threat count, legal-action count, legal-action enumeration, and successor state. Each decision state belongs to [c:gold]exactly one row family[/c], which keeps a position from being both trained and probed.\n\n{{visual:Table 2}}\n\nEvery answer has an executable criterion. Enumeration requires the exact legal-action set, successor prediction requires the state produced by applying the transition, and the counting questions are recomputed from the restored board. The supervision is **auditable by construction**, not by how convincing it sounds."
      },
      {
        "id": "spsd-what-a-training-example-looks-like",
        "title": "What one training example looks like",
        "body": "A linearized trace is easier to judge when the position, the search behind it and the rendered chain sit side by side. The explorer below does that on rows taken straight from the released corpus: pick a game, a rule variant and a task, then step through what turns one decision state into one training example.\n\nEach game is a standard two-player game $(\\mathcal{S}, \\mathcal{A}, P, r)$: states, actions, a transition rule, and a reward at the end. What matters for the data is that every part is **executable**: from a state $s$ the simulator lists exactly which actions are legal, $\\mathcal{A}(s) \\subseteq \\mathcal{A}$, and applies one to get the next state, $s' = P(s, a)$.\n\n:::key Row families\n[c:red]**Move-choice rows**[/c] ask for the expert's action. [c:blue]**State-question rows**[/c] ask what the board *says*: occupancy, legality, threat count, legal-action count, the exact legal set in display order, and the successor of a replayed transition.\n\nA decision state belongs to **exactly one** family, so a position is never both trained on and probed.\n:::\n\n{{explorer:spsd}}\n\n### Where the supervision comes from\n\nThe **search network** stage shows the frozen expert at work. It evaluates a state twice over, producing a [c:pink]policy prior[/c] $\\pi(a \\mid s)$ over legal actions and a [c:cyan]value[/c] $v(s) \\in [-1, 1]$ for the eventual outcome. It then spends $N = 50$ simulations, each descending the tree by\n\n$$a^{\\star} = \\arg\\max_{a \\in \\mathcal{A}(s)} \\left[\\, \\underbrace{Q(s,a)}_{\\text{what looks good}} \\;+\\; c \\cdot \\underbrace{\\pi(a \\mid s) \\frac{\\sqrt{\\textstyle\\sum_b N(s,b)}}{1 + N(s,a)}}_{\\text{what is under-examined}} \\,\\right]$$\n\nEvery node in the explorer's tree is a **real successor**: the board you see at $s^1$ is what the environment produces from $s^0$ under that edge's action, [c:gold]with the changed cells marked in gold[/c]. Hovering any node, edge or table row lights up the same action everywhere.\n\n:::claim Visit counts as supervision\nWhen the budget is exhausted, $N(s,a)$ over the legal set records how the expert distributed its search. That ranking is precisely what a single labelled move discards, and the linearized chain **retains the part of it replay can verify**.\n:::\n\n### What the student is trained on\n\nAlmost none of the search reaches the student. It sees the board, the legal handles, and one answer, with [c:blue]no priors, no values, no visit counts, and no chain[/c]. It is trained on\n\n$$\\mathcal{L}(\\theta) = -\\mathbb{E}_{(x, y) \\sim \\mathcal{D}} \\big[\\log p_\\theta(y \\mid x)\\big]$$\n\n:::claim Search cost at corpus construction\nThe tree is computed once, during corpus construction. **Expert strength and simulation budget are therefore parameters of the dataset** rather than costs at inference, and a stronger teacher yields better supervision without slowing the student.\n:::\n\n### Questions that cannot be talked around\n\nState questions run the other direction: each answer is recomputed from the restored position rather than asserted. An enumeration row is matched against $\\mathcal{A}(s)$ in display order, a counting row is recounted, a successor row is obtained by applying $P(s,a)$. A chain that merely *sounds* convincing cannot pass.\n\nThe explorer deliberately shows the harder rows. A board on which every action is legal teaches nothing, and [c:gold]89% of enumeration rows in the corpus are of that kind[/c].\n\n:::note Rule variants\nSwitching the rule variant while holding the game fixed changes scoring, openings, movement or winning conditions while **preserving board topology, observation shape and action-space size**.\n\nCorner-locked Tic-Tac-Chess and orthogonal-only Connect4 are [c:red]different games to plan in[/c] and [c:green]identical games to read[/c].\n:::"
      },
      {
        "id": "spsd-how-transfer-is-learned",
        "title": "Game supervision transfers to mathematics",
        "body": "Three post-training paths use the same corpus, so the **origin of the supervision is the only thing that differs**.\n\n- [c:red]**SFT**[/c] trains directly on the rendered chain and answer.\n- [c:green]**OPSD**[/c] lets the student sample its own response while a frozen teacher, conditioned on the expert trace as privileged context, supplies token-level guidance along that response. The student's own prompt contains only the board and the task.\n- [c:blue]**RuleBot-Distill**[/c] keeps the OPSD procedure unchanged and swaps the search-derived corpus for completions from a fixed heuristic policy.\n\nEvaluation covers 15 held-out Ludii games, disjoint from training, and six mathematics benchmarks absent from the game supervision: MATH500, AIME24, AIME25, AMC23, Olympiad Bench, and Minerva Math.\n\n{{visual:Table 1}}\n\n:::claim Results on Qwen3-4B-Base\nOn Qwen3-4B-Base, **OPSD triples the held-out-game win rate from [c:red]15%[/c] to [c:green]45%[/c]**, lifts FIDE from 15.0 to 39.9, and produces legal actions in 75.1% of evaluated positions.\n\nThe same training raises the mathematics mean from [c:red]24.1[/c] to [c:green]36.6[/c], improving **every individual benchmark**. A single corpus of search records yields gains in the source domain and in a reasoning domain absent from training.\n:::\n\nFitting the rendered chains directly does not achieve this. SFT reaches a mathematics mean of 30.3 while its legality [c:red]falls to 34.9%[/c]: the model gains on external tasks while **losing command of the action interface it was trained on**. Preserving what the student can already do turns out to be part of the result, not a side condition.\n\nThinking configuration reshapes the picture on Qwen3-8B. With thinking enabled, mathematics is near saturation for every condition and the separation moves to game play: OPSD reaches the strongest trained held-out-game result, [c:green]59.5 FIDE and 62.7% wins[/c], with complete action legality. With thinking disabled, search-derived knowledge lifts both evaluations. Llama-3.1-8B marks the boundary of the effect, where the two expert sources produce the same game outcomes and neither moves the mathematics mean.\n\n:::note Comparison with the heuristic teacher\nAgainst the heuristic teacher on Qwen3-4B-Base, OPSD raises wins from 35% to **45%** and mathematics from 33.9 to **36.6**, leading on every aggregate at once.\n\nRuleBot-Distill retains the best Olympiad Bench score (35.7 against 35.1), so a verified action carries some value on its own. It does not, however, carry the [c:pink]ranked alternatives[/c] and [c:cyan]replayable consequences[/c] behind the broader gain.\n:::"
      },
      {
        "id": "spsd-retaining-gains",
        "title": "OPSD keeps improving after SFT peaks",
        "body": "The endpoint ranking hides how each method got there. Tracking Qwen3-4B-Base across training makes the difference a matter of [c:gold]**retention**[/c] rather than speed. Game strength uses the FIDE score, which credits a loss 0, a draw 0.5, and a win 1.\n\n{{visual:Figure 3}}\n\n:::warn Degradation under SFT\nSFT reaches the strongest early mathematics checkpoint, **30.3 at step 100**. By step 1000 it is [c:red]23.1[/c], with legality sliding from 34.9% to 25.0% and FIDE returning to its **untrained level**.\n\nRepeatedly fitting a fixed rendered response narrows the model on the game interface and on the separate reasoning tasks at the same time.\n:::\n\nOPSD improves late instead. Attaching the teacher's privileged evidence to [c:green]**student-generated prefixes**[/c] keeps the update target on the responses the current model actually produces, and its curves climb through the final checkpoint on every axis.\n\nVarying the rules pushes this further. A separate pool of five base games with nine variants each yields **50 environments** that change scoring, openings, movement, or winning conditions while preserving board topology, observation shape, and action-space size.\n\n:::claim Variant-enriched curriculum\n**The variant-enriched curriculum leads at every evaluated checkpoint**, finishing at [c:green]42.4 FIDE, 77.3% legality, 39.2 mathematics[/c] against 39.9, 75.1%, and 36.6 for the base curriculum.\n\nDiversifying the decisions a search expert is asked to explain improves what survives to the end of training.\n:::"
      },
      {
    ...
    "abstractRich": "Improving reasoning abilities in Large Language Models (LLMs) requires **high-quality data that exposes difficult decisions, competing alternatives, and their consequences**. [c:red]Data scarcity is driven by the low quality of synthetic data and the cost of human labeling.[/c] We introduce **Self-Play Search Distillation (SPSD)**, a framework for generating superhuman synthetic data via self-play of MuZero-like networks trained on board games. SPSD uses [c:gold]executable environments to turn search into structured reasoning problems[/c]. At each state, the expert identifies *a preferred decision, plausible alternatives, plausible opponent replies, and value estimates*. By converting the self-play search records into superhuman chains-of-thought, we train LLMs with [c:blue]environment-grounded supervision[/c]. Although trained only on self-play search records, **SPSD transfers to unseen mathematics**. On Qwen3-4B-Base, it [c:green]raises the mean over six mathematics benchmarks from 24.1 to 36.6[/c] while [c:green]increasing the held-out-game win rate from 15% to 45%[/c]. SPSD offers an annotation-efficient way to create high-quality synthetic data for improving LLM performance in reasoning tasks."
  },
  {
    "id": "jab",
