    ...
    "sections": [
      {
        ...
        "body": "Search-based game experts produce a useful by-product: a record of the alternatives they considered and the consequences they explored. **Self-Play Search Distillation (SPSD)** converts those records into reasoning supervision for language models. A training example can explain why an action is promising, how an opponent could respond, and what happens along a competing continuation.\n\nBoard games give us an executable setting for constructing this data. The simulator determines which actions are legal, applies each move, and checks terminal outcomes. The expert supplies the decisions and comparisons, while replay supplies the facts used to explain them.\n\n{{visual:Figure 1}}\n\nGame competence broadly tracks performance on the reasoning benchmarks as model scale increases. Even frontier models leave substantial room for improvement on the board-game evaluation. These environments require a model to maintain state and anticipate the consequences of its actions through a sequence of decisions.\n\nThis motivates our transfer experiment: train on game-derived supervision, then evaluate on **unseen games and mathematics**. Figure 1 provides the cross-model context using GPQA-Diamond and Humanity's Last Exam. The post-training experiments below use a separate suite of six mathematics benchmarks."
      },
      {
        "id": "spsd-search-to-supervision",
        "title": "Replay turns search records into training data",
        "body": "We first train a MuZero-family expert through self-play and freeze its parameters. At each selected state, it runs 50 Monte Carlo tree search simulations and exports the chosen action, the visit-based ranking of alternatives, value estimates, and retained continuations. This separates expert training from LLM post-training and makes the resulting corpus reusable across students.\n\nThe simulator then restores the decision state and replays the selected move, a retained alternative, and an available opponent reply. The renderer uses these verified transitions to construct a compact reasoning chain ending in the expert's selected action.\n\n{{visual:Figure 2}}\n\nThe retained alternative gives the example its comparative content. A student can see how two plausible moves change the board and what an opponent can do next. A terminal claim such as a win or loss appears only when replay reaches that outcome. Invalid optional branches are discarded, and a record with an invalid selected action is rejected.\n\nAlongside move chains, we generate six kinds of **state questions**: occupancy, legality, threat count, legal-action count, legal-action enumeration, and successor-state prediction. These exercise the state-tracking operations needed to interpret a search trace.\n\n{{visual:Table 2}}\n\nThe verification rule follows the task. Action enumeration requires the exact legal-action set. Successor prediction requires the state produced by executing the move. Occupancy and count questions use values computed from the restored board. Some legal-action questions can be answered directly from the prompt's visible options, while successor prediction requires applying the transition rules."
      },
      {
        "id": "spsd-how-transfer-is-learned",
        "title": "Game supervision transfers to mathematics",
        "body": "We compare three training paths. **Supervised fine-tuning (SFT)** trains directly on the rendered reasoning chain and answer. **On-policy self-distillation (OPSD)** has the student generate its own response. A fixed language-model teacher receives the expert trace as additional context and supplies token-level guidance along that response. The student learns from this guidance while its own prompt contains the game state and task.\n\nThis gives the teacher access to search evidence at the exact prefixes the student visits. RuleBot-Distill provides a simpler comparison, using a fixed heuristic policy to generate compact, rule-verified responses. The search pipeline adds ranked alternatives, values, and replayable continuations. Each policy also contributes its own distribution of states and action targets.\n\nEvaluation covers 15 held-out games and six mathematics benchmarks: MATH500, AIME24, AIME25, AMC23, Olympiad Bench, and Minerva Math. The mathematics tasks are absent from the game supervision.\n\n{{visual:Table 1}}\n\nFor **Qwen3-4B-Base**, OPSD raises the mathematics mean from **24.1 to 36.3** and the held-out-game win rate from **15% to 45%**. It improves every mathematics benchmark over the base model and RuleBot-Distill. Move legality also rises from 65% to 75%. The same training data therefore supports gains in both the source domain and the external reasoning tasks.\n\nThe contrast with SFT is sharp. Qwen3-4B-Base reaches a mathematics mean of 30.3 under SFT, but its legality falls to 35%. On Qwen3-8B with native thinking enabled, OPSD raises the mathematics mean from 59.4 to 62.3 while preserving 100% legality. SFT reduces that model's mathematics mean to 6.8. Preserving the student's existing capabilities becomes a central part of successful transfer.\n\nThe response depends on the backbone. Llama-3.1-8B finishes at 19.2 on mathematics against a 19.5 baseline, and OPSD ties RuleBot-Distill on the reported game metrics. The strongest joint gains occur in the Qwen configurations."
      },
      {
        "id": "spsd-retaining-gains",
        "title": "OPSD keeps improving after SFT peaks",
        "body": "The Qwen3-4B-Base learning curves show how the final ranking develops. We track mathematics, move legality, and the **FIDE score**, which assigns 0 to a loss, 0.5 to a draw, and 1 to a win.\n\n{{visual:Figure 3}}\n\nSFT reaches the strongest early mathematics checkpoint, then loses ground as training continues. Its legality and game score also deteriorate. RuleBot-Distill improves early and gives back part of those gains later. OPSD sustains progress in the game metrics, and its base curriculum continues improving on mathematics through the final checkpoint.\n\nThese trajectories explain why the training objective matters. SFT repeatedly fits a fixed rendered response. OPSD attaches the teacher's privileged evidence to **student-generated prefixes**, keeping the guidance connected to the responses the current model actually produces. The later improvements are consistent with that training design.\n\nWe also vary game rules while preserving the board topology, observation shape, and action-space size. Changes to scoring, movement, openings, or winning conditions expose the student to additional decision patterns through the same interface. The variant-enriched mathematics curve peaks before the end and then recedes slightly, while base OPSD continues rising. Curriculum composition and checkpoint choice both affect the gains retained at the end of training."
      },
      {
        "id": "spsd-agreement-is-not-quality",
        "title": "Winning decisions align with MCTS",
        "body": "We evaluate each model's choices with a fresh 50-simulation MCTS search. **Q50** records the expert's estimated value of the selected action. **Oracle@50** records how often the model selects the action with the highest root-visit count. Together, they let us examine how closely successful play follows the search expert's decisions.\n\n{{visual:Figure 4}}\n\n**Closer alignment with MCTS accompanies stronger game performance.** Figure 4 connects successful play with choosing valuable actions and following the search expert's preferences. Across all four systems, wins are scarce when both signals are weak. For SPSD-Qwen3-4B-Base, the concentrated high-win region brings action value and expert agreement together.\n\nWe call this **reasoning sharpness**: selecting valuable actions and following the expert when its preference is informative. SPSD's supervision includes both the preferred move and the alternatives considered during search, giving the student examples of how good decisions emerge from those comparisons.\n\nExpert strength, search budget, and environmental diversity become adjustable inputs to corpus construction. The search runs during data generation. At inference, the trained LLM receives the rules and board state and chooses its action directly."
      }
    ],
    "links": {},
    "visualSelection": {
